...
language sql
as $$
  select upper('CN' || replace(gen_random_uuid()::text, '-', ''));
$$;
...
begin
  ...
  values (
    ...
    coalesce(new.raw_user_meta_data->>'invitation_code', public.generate_referral_code()),
    null,
  ...
  perform public.process_referral(new.id, new.raw_user_meta_data->>'invitation_code');
